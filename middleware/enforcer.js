  ...
  return function (ctx, next) {
    ...
    if (ctx.state.kauth && ctx.state.kauth.grant) {
      if (
        handlePermissions(expectedPermissions, function (resource, scope) {
          if (
            !ctx.state.kauth.grant.access_token.hasPermission(resource, scope)
          ) {
            return false;
          }
        })
    ...
    if (config.claims) {
      const claims = config.claims(ctx.request);
    ...
    if (config.response_mode === "permissions") {
      return keycloak
        .checkPermissions(authzRequest, ctx.request, function (permissions) {
          if (
            handlePermissions(expectedPermissions, function (resource, scope) {
      ...
      return keycloak
        .checkPermissions(authzRequest, ctx.request)
        .then(function (grant) {
          if (
            handlePermissions(expectedPermissions, function (resource, scope) {
